...
/**
 * GET /api/v1/kitchens (admin)
 * Get all kitchens with pagination and filters.
 */
export const getAllKitchens = asyncHandler(async (req, res) => {
  const { page, limit, status } = req.query;

  const result = await kitchenService.getAllKitchens({
...
/**
 * PATCH /api/v1/kitchens/:id/approve (admin)
 */
export const approveKitchen = asyncHandler(async (req, res) => {
  const kitchen = await kitchenService.updateKitchenApproval(
    req.params.id,
    true
  );
...
/**
 * PATCH /api/v1/kitchens/:id/reject (admin)
 */
export const rejectKitchen = asyncHandler(async (req, res) => {
  const kitchen = await kitchenService.updateKitchenApproval(
    req.params.id,
    false
  );
